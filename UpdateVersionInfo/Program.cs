...
namespace UpdateVersionInfo
{
	class Program
	{
		...
		private static void UpdateAndroidVersionInfo(string path, Version version)
		{
			...
			string versionName = version.Major + "." + version.Minor;
		...
		private static bool IsValidTouchPList(string path)
		{
			...
			try
			{
				...
				{
					XElement? shortVersionElement = doc.XPathSelectElement("plist/dict/key[string()='CFBundleShortVersionString']");
					if (shortVersionElement is not null)
