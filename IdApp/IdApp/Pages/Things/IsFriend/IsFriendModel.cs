...
namespace IdApp.Pages.Things.IsFriend
{
	...
	public class IsFriendModel : XmppViewModel
	{
			...
			: base()
		{
			...
			this.RejectCommand = new Command(async _ => this.Reject());
		...
		private void Accept()
		{
			this.XmppService.IoT.ProvisioningClient.IsFriendResponse(this.ProvisioningService, this.BareJid, this.RemoteJid, this.Key, true,
				this.GetRuleRange(), async (sender, e) =>
			{
				if (e.Ok)
				{
					...
					this.UiSerializer.BeginInvokeOnMainThread(async () => await this.UiSerializer.DisplayAlert(e.StanzaError ?? new Exception("Unable to accept request.")));
			}, null);
		}

		private void Reject()
		{
			this.XmppService.IoT.ProvisioningClient.IsFriendResponse(this.ProvisioningService, this.BareJid, this.RemoteJid, this.Key, false,
				this.GetRuleRange(), async (sender, e) =>
			{
				if (e.Ok)
				{
