...
There are two branches, plus release branches. The more 'static' branches are:
- `master`
- `develop`

The `develop` branch is the default branch. When working on a new release,
create a version specific branch off of `develop`.

Once it is implemented and tested, merge it back to `develop`.
