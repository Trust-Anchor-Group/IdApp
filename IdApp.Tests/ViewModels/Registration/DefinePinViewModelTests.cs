using IdApp.Pages.Registration.DefinePin;
using Moq;
    ...
    public class DefinePinViewModelTests : ViewModelTests<DefinePinViewModel>
    {
        ...
        public void When_ContinueIsClicked_AndPinIsEmpty_ThenAWarningIsDisplayed(string pin)
        {
            Given(AViewModel)
                ...
                .ThenAssert(() => dispatcher.Verify(x => x.DisplayAlert(AppResources.ErrorTitle, string.Format(AppResources.PinTooShort, Constants.Authentication.MinPinLength)), Times.Once))
        ...
        public void When_ContinueIsClicked_AndPinContainsWhitespace_ThenAWarningIsDisplayed(string pin)
        {
            Given(AViewModel)
                .And(vm => vm.Pin = pin)
                .And(vm => vm.ContinueCommand.IsExecuted())
                .ThenAssert(() => dispatcher.Verify(x => x.DisplayAlert(AppResources.ErrorTitle, AppResources.PinMustNotIncludeWhitespace), Times.Once))
