...
namespace Tag.Sdk.Core.Tests.Services
{
    public class NetworkServiceTests
    {
            ...
            where TExceptionRethrown : Exception
        {
            ...
            try
            {
                await this.networkService.Request(TestFuncThatThrows, rethrowException, displayAlert, nameof(VerifyRequest));
            }
            catch (Exception ex)
            {
            ...
            //this.dispatcher.Verify(x => x.DisplayAlert(It.Is<string>(title => title.Equals(AppResources.ErrorTitle)), It.IsAny<string>()), displayAlertCount);
